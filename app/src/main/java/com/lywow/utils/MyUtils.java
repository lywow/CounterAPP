...
public class MyUtils {
    ...
    public static boolean checkNumberOnly(String str){
        for(int i=0;i<str.length()-1;i++){
            if(!isCharNum(str.charAt(i))&&str.charAt(i)!='.'){
                return false;
            }
        }
        return true;
    ...
    public static boolean checkFormat(String str) {
        ...
        // 校验结尾是否是数字或“)”
        //if(!(isCharNum(str.charAt(str.length()-2)) || str.charAt(str.length()-2) == '）')) {
            //return false;
        //}
        ...
        for (int i = 1; i < str.length() - 1; i++) {
            ...
            if (!(isCharNum(c))) {
                ...
                if (c == '.') {
                    ...
                    if(pointError(str,i+1,1) || pointError(str,i-1,-1)){
                        System.out.println("********************************-----------------------------");
                        return false;
                    }
                }
    ...
    public static boolean pointError(String str,int index,int sign){
        if(index==-1||index==str.length()){
            return false;
        }
        if(isCharNum(str.charAt(index))){
            return pointError(str,index+sign,sign);
